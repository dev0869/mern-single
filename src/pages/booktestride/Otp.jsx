  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (res.data.sucess) {
        ...
        const res2 = await addtestride(data);
        if (res2.bookingid) {
          toast.success("You booking for test bike is sucessfully Submitted");
          localStorage.setItem("bkid", res2.bookingid);
          navigate("/testbookride/success");
        } else {
          toast.error(res2);
          localStorage.setItem("bkid", res2);
          navigate("/testbookride/fail");
        }
      } else {
