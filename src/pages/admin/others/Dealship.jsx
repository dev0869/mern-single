...
import { deleteDealerShip } from "../api";

const DealerShipReq = () => {
  const dispatch = useDispatch();
  const booktestride = useSelector((st) => st.dealer.data);
  ...
  const ReadUser = ({ itm }) => {
    const merge = { ...itm, action: "view dealer" };
  ...
  const handledelete = async (itm) => {
    try {
      const res = await deleteDealerShip({ _id: itm._id });
      toast.success(res);
  ...
  return (
    <div className="flex flex-col col-12">
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <h1 className="h3 font-bold">DealerShip Requests</h1>
        <div style={{ display: "flex", flexDirection: "row", gap: "12px" }}>
          {/* <button style={{ width: "40px" }} onClick={Downloadpdf}>
            {" "}
            <img src={datas} alt="u9u9" />
          </button> */}
      ...
      <div className="col-12 border overflow-x-auto mt-5">
          ...
          <tbody>
            {currentItems.map((itm, i) => (
              <tr key={itm._id}>
                ...
                <td>{itm.number}</td>
                <td>{itm.businessname}</td>
                <td>{itm.businesstype}</td>
