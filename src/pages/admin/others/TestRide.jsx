...
import { deletetestRide } from "../api";
...
const TestRide = () => {
  ...
  const EditUser = ({ itm }) => {
    const merge = { ...itm, action: "edit book" };
    dispatch(readUser(merge));
    navigate("/testing");
  };
  ...
  const handledelete = async (itm) => {
    try {
      const res = await deletetestRide({ _id: itm._id });
      toast.success(res);
  ...
  return (
    <div className="flex flex-col col-12">
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <h1 className="h3 font-bold">Quotation Request</h1>
