...
import { addcallback } from "../admin/api";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (res.data.sucess) {
        ...
        const res2 = await addcallback(data);
        if (res2.bookingid) {
          toast.success("You booking for test bike is sucessfully Submitted");
          localStorage.setItem("cbid", res2.bookingid);
          navigate("/callback/success");
        } else {
          toast.error(res2);
          localStorage.setItem("cbid", res2);
          navigate("/callback/fail");
        }
      } else {
