...
import { adddealerrshipRequest } from "../admin/api";
const DealerOtp = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (res.data.sucess) {
        ...
        const res2 = await adddealerrshipRequest(data);
        if (res2.bookingid) {
          toast.success("You booking for test bike is sucessfully Submitted");
          localStorage.setItem("drid", res2.bookingid);
          navigate("/dealer/success");
        } else {
          toast.error(res2);
          localStorage.setItem("drid", res2);
          navigate("/dealer/fail");
        }
      } else {
